...
final class _EsmEmitter {
  ...
  EsmBackendResult emit() {
    ...
    _prepareExtensionTypeDescriptors(module);
    _prepareClassRuntimeNames(module.classRuntime);
  ...
  void _prepareClassRuntimeNames(EsmClassRuntimePlan runtime) {
    final sortedClasses = runtime.classes.toList()
      ..sort((left, right) => _className(left).compareTo(_className(right)));
    for (final klass in sortedClasses) {
      final interfaces = runtime.interfaceMarkersFor(klass);
  ...
  void _prepareExtensionTypeDescriptors(EsmModulePlan module) {
    for (final library in module.libraries) {
      for (final extensionType in library.extensionTypes) {
        _declareExtensionTypeDescriptors(extensionType.node);
      }
    }
  }
