...
void main() {
  test('compiles supported Kernel through the new compiler core', () {
    ...
    expect(result.runtime?.module.runtimeHelpers, isEmpty);
...
''');
  ...
  test('runtime linker resolves helper declarations before codegen', () {
    ...
    final linked = const RuntimeLinkerStage().link(
      _normalizationForModule(module),
    );

    expect(linked.linkedHelpers, [EsmRuntimeHelper.print]);
    expect(linked.module.runtimeHelpers, isEmpty);
    expect(linked.module.items, hasLength(2));
    final helper = linked.module.items.first;
    expect(helper, isA<EsmRuntimeHelperDeclarationIr>());
    expect((helper as EsmRuntimeHelperDeclarationIr).name, '__dartPrint');
    expect(helper.source, startsWith('function __dartPrint'));
    expect(helper.source, isNot(startsWith('\n')));
...
NormalizationResult _normalizationForModule(EsmModuleIr module) {
  final libraryUri = Uri.parse('package:sample/main.dart');
  final library = k.Library(libraryUri, fileUri: libraryUri);
  ...
  return NormalizationResult(
    lowering: LoweringResult(semantic: semantic, module: module),
    module: module,
