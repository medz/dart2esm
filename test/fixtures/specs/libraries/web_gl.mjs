function __dartStr(value) {
  if (value == null) return "null";
  if (Array.isArray(value)) {
    return "[" + value.map(__dartStr).join(", ") + "]";
  }
  if (value instanceof Set) {
    return "{" + Array.from(value).map(__dartStr).join(", ") + "}";
  }
  if (value instanceof Map) {
    return "{" + Array.from(value, ([key, entryValue]) => __dartStr(key) + ": " + __dartStr(entryValue)).join(", ") + "}";
  }
  if (typeof value === "object") {
    const toString = value.toString;
    if (typeof toString === "function" && toString !== Object.prototype.toString) {
      return String(toString.call(value));
    }
  }
  return String(value);
}
function __dartPrint(value) {
  console.log(__dartStr(value));
}
function __dartAs(value, test, typeName) {
  if (test(value)) return value;
  throw new TypeError("Type cast failed: expected " + typeName);
}
function __dartCanvasElement(width, height) {
  const document = globalThis.document;
  if (document == null || typeof document.createElement !== "function") throw new TypeError("CanvasElement requires document.createElement");
  const canvas = document.createElement("canvas");
  if (width != null) canvas.width = width;
  if (height != null) canvas.height = height;
  return canvas;
}
function __dartCanvasGetContext3d(canvas, options) {
  const context = canvas.getContext("webgl", options);
  return context ?? canvas.getContext("experimental-webgl", options);
}

// Generated by dart2esm.

export function hide(value) {
  return value;
}

export function main() {
  const canvas = __dartCanvasElement(16, 8);
  const generic = canvas.getContext("webgl");
  const context = __dartCanvasGetContext3d(canvas, { alpha: true, depth: true, stencil: false, antialias: true, premultipliedAlpha: true, preserveDrawingBuffer: false });
  ((context)?.clearColor(0, 0, 0, 1) ?? null);
  ((context)?.clear(16384.0) ?? null);
  const mask = (16384.0 | 4.0);
  const hiddenContext = hide(context);
  const castContext = __dartAs(hiddenContext, value => (value === null || (typeof globalThis["WebGLRenderingContext"] === "function" && value instanceof globalThis["WebGLRenderingContext"])), "RenderingContext?");
  __dartPrint("webgl " + __dartStr((!!globalThis.window?.WebGLRenderingContext)) + " " + __dartStr(mask) + " " + __dartStr(canvas.width) + " " + __dartStr(canvas.height) + " " + __dartStr(!((generic === null))) + " " + __dartStr(!((context === null))) + " " + __dartStr((typeof globalThis["WebGLRenderingContext"] === "function" && hiddenContext instanceof globalThis["WebGLRenderingContext"])) + " " + __dartStr(!((castContext === null))));
}

main();
