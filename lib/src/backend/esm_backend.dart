...
final class _EsmEmitter {
  ...
  void _declareTopLevelNames(EsmModulePlan module) {
    for (final library in module.libraries) {
      for (final field in library.fields) {
      ...
      for (final klass in library.classes) {
        _classNames[klass.node] = _freshName(klass.node.name);
      ...
      for (final extensionType in library.extensionTypes) {
        _extensionTypeNames[extensionType.node] = _freshName(
          extensionType.node.name,
        );
        _declareExtensionTypeDescriptors(extensionType.node);
      }
      for (final procedure in library.procedures) {
        _procedureNames[procedure.node] = _freshName(procedure.node.name.text);
      }
    }
  }
