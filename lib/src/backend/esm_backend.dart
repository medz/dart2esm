...
final class _EsmEmitter {
  ...
  EsmBackendResult emit() {
    ...
    final exportNamesByLibrary = <k.Library, Set<String>>{
      for (final library in libraries)
        library: identical(library, mainLibrary)
            ? mainExportNames
            : localExports[library] ?? const <String>{},
    };
    _emitLibraryTypes(libraries, exportNamesByLibrary);
    for (final library in libraries) {
      _emitLibraryMembers(library, exportNames: exportNamesByLibrary[library]!);
    }
  ...
  List<k.Library> _emittableLibraries(k.Library mainLibrary) {
    final libraries = <k.Library>[
      for (final library in component.libraries)
        if (identical(library, mainLibrary) ||
            library.importUri.scheme != 'dart')
          library,
    ];
    final emittable = libraries.toSet();
    final pending = libraries..sort(_compareLibraries);
    ...
    void visit(k.Library library) {
      ...
      for (final dependency in library.dependencies) {
        ...
        if (target != null && emittable.contains(target)) {
  ...
  void _emitLibraryTypes(
    ...
    for (final library in libraries) {
      final exportNames = exportNamesByLibrary[library];
      for (final extensionType in library.extensionTypeDeclarations) {
  ...
  List<k.Class> _classesInGlobalEmitOrder(List<k.Library> libraries) {
    final classes = {for (final library in libraries) ...library.classes};
