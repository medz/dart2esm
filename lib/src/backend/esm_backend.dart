...
final class _EsmProgramPlan {
  const _EsmProgramPlan({
...
final class _EsmReachability {
  ...
  final bool Function(k.Procedure procedure) isEmittableTopLevelProcedure;
...
final class _EsmEmitter {
  ...
  EsmBackendResult emit() {
    ...
    final plan = _EsmReachability(
      component,
      main: main,
      exportNamesByLibrary: exportNamesByLibrary,
      isEmittableTopLevelProcedure: _isEmittableTopLevelProcedure,
    ).compute();
    final libraries = _orderedLibraries(plan.libraries);
    for (final library in libraries) {
      _declareTopLevelNames(library, plan);
    }
    _prepareClassRuntime(plan.classes);

    writeln('// Generated by dart2esm.');
    writeln();
    _emitLibraryTypes(libraries, plan, exportNamesByLibrary);
    for (final library in libraries) {
      _emitLibraryMembers(
        library,
        plan,
        exportNames: exportNamesByLibrary[library] ?? const <String>{},
      );
    }
  ...
  List<k.Library> _orderedLibraries(Set<k.Library> libraries) {
    ...
    void visit(k.Library library) {
      ...
      for (final dependency in library.dependencies) {
        final target = _dependencyTargetLibrary(dependency);
  ...
  k.Library? _dependencyTargetLibrary(k.LibraryDependency dependency) {
  ...
  Iterable<String> _libraryDeclarationNames(k.Library library) sync* {
    ...
    for (final procedure in library.procedures) {
      if (_isEmittableTopLevelProcedure(procedure) &&
  ...
  bool _isEmittableTopLevelProcedure(k.Procedure procedure) {
  ...
  void _prepareClassRuntime(Set<k.Class> classes) {
    ...
    for (final klass in sortedClasses) {
      final superclass = _jsInterfaceSuperclassFor(klass, classes);
      if (superclass != null) {
  ...
  void _emitLibraryTypes(
    List<k.Library> libraries,
    _EsmProgramPlan plan,
    Map<k.Library, Set<String>> exportNamesByLibrary,
  ) {
    final classes = _classesInGlobalEmitOrder(libraries, plan.classes);
    ...
    for (final library in libraries) {
      final exportNames = exportNamesByLibrary[library] ?? const <String>{};
      for (final extensionType in library.extensionTypeDeclarations) {
        if (!plan.extensionTypes.contains(extensionType)) {
          continue;
        }
  ...
  void _emitLibraryMembers(
    k.Library library,
    _EsmProgramPlan plan, {
    required Set<String>? exportNames,
  }) {
    for (final field in library.fields) {
      if (field.isExtensionTypeMember || !plan.topLevelFields.contains(field)) {
        continue;
      }
      _emitTopLevelField(
        field,
    ...
    for (final procedure in library.procedures) {
      if (plan.topLevelProcedures.contains(procedure) &&
          _isEmittableTopLevelProcedure(procedure)) {
        _emitProcedure(
          procedure,
  ...
  void _declareTopLevelNames(k.Library library, _EsmProgramPlan plan) {
    for (final field in library.fields) {
      if (plan.topLevelFields.contains(field)) {
        _fieldNames[field] = _freshTopLevelFieldNames(field.name.text);
      }
    }
    for (final klass in library.classes) {
      if (!plan.classes.contains(klass)) {
        continue;
      }
  ...
  k.Class? _jsInterfaceSuperclassFor(k.Class klass, Set<k.Class> classes) {
    ...
    return _localClassFromSupertype(klass.mixedInType, classes);
  }
  ...
  String _emitStaticSet(k.StaticSet expression) {
    ...
    if (target is k.Procedure &&
        ...
        _procedureNames.containsKey(target)) {
  ...
  String _emitInstanceInvocation(k.InstanceInvocation expression) {
    ...
    if (expression.arguments.named.isEmpty &&
        name == 'compareTo' &&
        positionalArgs.length == 1 &&
        _isCoreMember(target, 'Comparable', name)) {
      _usedHelpers.add('__dartCompare');
