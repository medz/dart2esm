...
''');
    ...
    expect(code, contains('console.log(__dartStr(value));'));
    expect(code, contains('function __dartStr(value) {'));
    expect(code, contains(r'  __dartPrint(`new ${__dartStr(target)}`);'));
...
''');
      ...
      expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
      final code = output.readAsStringSync();
      expect(code, contains('export let counter = 1;'));
      expect(code, contains('export const seed = 3;'));
      expect(code, contains('  counter = counter + seed;'));
      expect(
        code,
        contains(r'  __dartPrint(`counter ${__dartStr(counter)}`);'),
      );
...
''');
  ...
  test('preserves inherited accessor pairs in JS classes', () async {
    final tempDir = await Directory.systemTemp.createTemp(
      'dart2esm-inherited-accessors-core-',
    );
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final input = File(p.join(tempDir.path, 'main.dart'))
      ..writeAsStringSync('''
...
class SetterOnly extends Base {
  set value(int next) {
    super.value = next + 1;
  }
}

class GetterOnly extends Base {
  int get value => super.value + 10;
}

void main() {
  final setterOnly = SetterOnly();
  setterOnly.value = 4;
  final getterOnly = GetterOnly();
  getterOnly.value = 5;
  print('accessors \${setterOnly.value} \${getterOnly.value}');
}
''');
    final output = File(p.join(tempDir.path, 'main.mjs'));
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    await _expectNodeOutput(output, 'accessors 5 15\n');
  });

  test('captures this lexically in Dart closures', () async {
    final tempDir = await Directory.systemTemp.createTemp(
      'dart2esm-closure-this-core-',
    );
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final input = File(p.join(tempDir.path, 'main.dart'))
      ..writeAsStringSync('''
class Holder {
  ...
  int readWithClosure() {
    int local = 2;
    final read = () => value + local;
    return read();
  }
}

void main() {
  print('closure \${Holder(5).readWithClosure()}');
}
''');
    final output = File(p.join(tempDir.path, 'main.mjs'));
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    await _expectNodeOutput(output, 'closure 7\n');
  });

  test('defines instance fields over inherited abstract getters', () async {
    final tempDir = await Directory.systemTemp.createTemp(
      'dart2esm-field-over-getter-core-',
    );
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final input = File(p.join(tempDir.path, 'main.dart'))
      ..writeAsStringSync('''
...
class MappingStartEvent extends ValueEvent {
  final String? anchor;

  MappingStartEvent({this.anchor});
}

void main() {
  final event = MappingStartEvent(anchor: 'root');
  print('field \${event.anchor}');
}
''');
    final output = File(p.join(tempDir.path, 'main.mjs'));
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    await _expectNodeOutput(output, 'field root\n');
  });

  test('supports concrete classes used as implemented interfaces', () async {
    final tempDir = await Directory.systemTemp.createTemp(
      'dart2esm-concrete-interface-core-',
    );
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final input = File(p.join(tempDir.path, 'main.dart'))
      ..writeAsStringSync('''
...
class ImplementsConcrete implements ConcreteInterface {
  @override
  final int value;

  ImplementsConcrete(this.value);
}

void main() {
  final value = ImplementsConcrete(7);
  final casted = value as ConcreteInterface;
  print('concrete \${value is ConcreteInterface} \${casted.value}');
}
''');
    final output = File(p.join(tempDir.path, 'main.mjs'));
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    await _expectNodeOutput(output, 'concrete true 7\n');
  });

  test('initializes fields in implicit default constructors', () async {
    final tempDir = await Directory.systemTemp.createTemp(
      'dart2esm-implicit-field-constructor-core-',
    );
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final input = File(p.join(tempDir.path, 'main.dart'))
      ..writeAsStringSync('''
...
void main() {
  ...
  print('implicit \${collector.items.length} \${collector.items.first}');
}
''');
    final output = File(p.join(tempDir.path, 'main.mjs'));
...
''');
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    final code = output.readAsStringSync();
    expect(code, contains('export class Counter {'));
    expect(code, contains('  add(amount) {'));
    expect(code, contains('  get doubled() {'));
    expect(
      code,
      contains(r'  __dartPrint(`value ${__dartStr(counter.value)}`);'),
    );
    expect(
      code,
      contains(r'  __dartPrint(`double ${__dartStr(counter.doubled)}`);'),
    );
...
''');
    ...
    expect(code, contains('    __dartPrint("then");'));
    expect(code, contains('    __dartPrint("else");'));
...
''');
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    final code = output.readAsStringSync();
    expect(code, contains('  while (i < 3) {'));
    expect(code, contains(r'    __dartPrint(`i ${__dartStr(i)}`);'));
    expect(code, contains('    i = i + 1;'));
...
''');
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    final code = output.readAsStringSync();
    expect(code, contains('  for (let i = 0; i < 4; i = i + 1) {'));
    expect(code, contains('    total = total + i;'));
    expect(code, contains(r'  __dartPrint(`total ${__dartStr(total)}`);'));
...
''');
    ...
    expect(code, contains(r'    __dartPrint(`do ${__dartStr(i)}`);'));
    expect(code, contains('  } while (i < 2);'));
...
''');
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    final code = output.readAsStringSync();
    expect(code, contains('    switch (value) {'));
    ...
    expect(
      code,
      contains(r'        __dartPrint(`other ${__dartStr(value)}`);'),
    );
...
class Items<E> extends ListBase<E> {
  Items(this._items);
...
void main() {
  List<int> values = Items<int>([3, 4]);
  values[1] = 9;
  print('listLike \${values[0]} \${values[1]}');
}
''');
    final output = File(p.join(tempDir.path, 'main.mjs'));
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    final code = output.readAsStringSync();
    expect(code, contains('__dartListLikeGet'));
    expect(code, contains('__dartListLikeSet'));
    await _expectSameDartAndNodeOutput(input, output);
  });
...
''');
  ...
  test('compiles BigInt helpers through the new core', () async {
    ...
    final tempDir = await Directory.systemTemp.createTemp(
      'dart2esm-bigint-core-',
    );
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final output = File(p.join(tempDir.path, 'bigint.mjs'));
  ...
  test('compiles dart:math helpers through the new core', () async {
    ...
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final output = File(p.join(tempDir.path, 'math.mjs'));
...
''');
  ...
  test('compiles enums through the new core', () async {
    ...
    for (final id in const ['enums/basic_enums', 'enums/enhanced_enums']) {
      final source = File(p.join(fixtureDir.path, '$id.dart'));
      final expected = File(p.join(fixtureDir.path, '$id.mjs'));
      final output = File(
        p.join(tempDir.path, '${id.replaceAll('/', '_')}.mjs'),
  ...
  test('local bindings avoid module global names', () async {
    final tempDir = await Directory.systemTemp.createTemp(
      'dart2esm-global-hygiene-',
    );
    addTearDown(() => tempDir.deleteSync(recursive: true));
    final helper = File(p.join(tempDir.path, 'helper.dart'))
      ..writeAsStringSync('''
String url(String value) => 'global \$value';
''');
    final input = File(p.join(tempDir.path, 'main.dart'))
      ..writeAsStringSync('''
import 'helper.dart' as p;
...
''');
    ...
    expect(result.success, isTrue, reason: result.diagnostics.join('\n'));
    final code = output.readAsStringSync();
    expect(code, contains('function url(value)'));
    expect(code, contains('const url_1 = url("ok");'));
    await _expectSameDartAndNodeOutput(input, output);
...
Future<void> _expectGoldenFixture(_GoldenFixture fixture) async {
  ...
  if (_isPendingNewCoreFixture(fixture.id)) {
    expect(
      result.success,
      isFalse,
      reason:
...
bool _isPendingNewCoreFixture(String fixtureId) {
  return _pendingNewCoreFixtureIds.contains(fixtureId);
}

const _pendingNewCoreFixtureIds = {
