...
EsmModulePlan buildEsmModulePlan({
  ...
  required EsmProgramPlan program,
  required Map<k.Library, Set<String>> exportNamesByLibrary,
}) {
  final classes = _classesInGlobalEmitOrder(orderedLibraries, program.classes)
      .map((klass) {
        final exportNames =
  ...
  return EsmLibraryPlan(
    ...
    classes: [
      for (final klass in library.classes)
        if (program.classes.contains(klass))
          EsmClassPlan(
            node: klass,
            export: _shouldExport(klass.name, exportNames),
          ),
    ...
    fields: [
      for (final field in library.fields)
        if (!field.isExtensionTypeMember &&
            program.topLevelFields.contains(field))
          EsmFieldPlan(
            node: field,
    ...
    procedures: [
      for (final procedure in library.procedures)
        if (program.topLevelProcedures.contains(procedure) &&
    ...
    extensionTypes: [
      for (final extensionType in library.extensionTypeDeclarations)
        if (program.extensionTypes.contains(extensionType))
