      ...
      for (; _sync_for_iterator.moveNext(); ) {
        {
          let element = _sync_for_iterator.current;
          {
            ...
            hash_1 = (hash_1 ^ (hash_1 >> 6));
          }
        }
      }
    }
    hash_1 = ((hash_1 + (hash_1 << 3)) & 2147483647);
    hash_1 = (hash_1 ^ (hash_1 >> 11));
...
class Highlighter {
                ...
                for (; _sync_for_iterator_1.moveNext(); ) {
                  {
                    ...
                    {
                      if ((lines.length === 0 || (lineNumber > lines[lines.length - 1].number))) {
                        {
    ...
    for (let i = 0; (i < this._lines.length); i = (i + 1)) {
      {
        ...
        if ((i > 0)) {
          {
            const lastLine = this._lines[(i - 1)];
        ...
        const primary = (__dartEquals(primaryIdx, (-1)) ? null : line.highlights[primaryIdx]);
        if (!((primary === null))) {
          {
  ...
  _writeLabel(highlight, highlightsByColumn, underlineLength) {
    ...
    const color = (highlight.isPrimary ? this._primaryColor : this._secondaryColor);
    this._colorize(() => { return this._buffer.write(" " + __dartStr(lines[0])); }, { color: color });
    this._buffer.writeln();
    {
...
function $Highlighter__($newTarget, _lines, _primaryColor, _secondaryColor) {
  ...
  $self._paddingBeforeSidebar = (1 + Math.max(__dartStr((_lines[_lines.length - 1].number + 1)).length, (Highlighter._contiguous(_lines) ? 0 : 3)));
...
class SourceFile {
  ...
  _isNearCachedLine(offset) {
    ...
    if ((offset < this._lineStarts[cachedLine])) {
      return false;
    }
    if (((cachedLine >= (this._lineStarts.length - 1)) || (offset < this._lineStarts[(cachedLine + 1)]))) {
      {
        return true;
      }
    }
    if (((cachedLine >= (this._lineStarts.length - 2)) || (offset < this._lineStarts[(cachedLine + 2)]))) {
      {
  ...
  getOffset(line, column = null) {
    ...
    const result = (this._lineStarts[line] + column);
    if (((result > this.length) || (((line + 1) < this.lines) && (result >= this._lineStarts[(line + 1)])))) {
      {
...
function $SourceFile__fromList($newTarget, decodedChars, { url: url_1 = null } = {}) {
  ...
  for (let i = 0; (i < $self._decodedChars.length); i = (i + 1)) {
    {
      let c = (() => { let v = $self._decodedChars; return (() => { let v_1 = i; return (() => { let v_2 = decodedChars[i]; return (() => { let v_3 = v[v_1] = v_2; return v_2; })(); })(); })(); })();
      if (__dartEquals(c, 13)) {
        {
          const j = (i + 1);
          if (((j >= decodedChars.length) || !(__dartEquals(decodedChars[j], 10)))) {
            c = 10;
          }
        }
      }
...
class LineScanner extends StringScanner {
      ...
      if ((newPosition > oldPosition)) {
        {
          ...
          if (newlines.length === 0) {
            ...
            {
              const offsetOfLastNewline = (oldPosition + newlines[newlines.length - 1].end);
  ...
  scan(pattern) {
    ...
    if (newlines.length === 0) {
      ...
      {
        this._column = (__dartNullCheck(__dartNullCheck(this.lastMatch)[0]).length - newlines[newlines.length - 1].end);
      }
    }
    return true;
  }
...
class EagerSpanScanner extends SpanScanner {
    ...
    if ((newPosition > oldPosition)) {
      {
        ...
        if (newlines.length === 0) {
          ...
          {
            this._column = (newPosition - newlines[newlines.length - 1].end);
          }
        }
      }
    } else {
  ...
  scan(pattern) {
    ...
    if (newlines.length === 0) {
      ...
      {
        this._column = (firstMatch.length - newlines[newlines.length - 1].end);
      }
    }
    return true;
  }
...
function SourceSpanWithContextExtension_subspan(_this, start, end = null) {
  ...
  return new SourceSpanWithContext(locations[0], locations[1], _this.text.substring(start, end), _this.context);
}
...
function SourceSpanExtension_subspan(_this, start, end = null) {
  ...
  const locations = subspanLocations(_this, start, end);
  return new SourceSpan(locations[0], locations[1], _this.text.substring(start, end));
}
